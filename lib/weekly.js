...
module.exports = async function weekly(command, message) {
  ...
  if (command.length == 2) {
    let week = [];
    try {
      const getWeeklyConfig = await WeeklyConf.query().findOne({ serverId: Number(message.guild.id) });
      week = [
    ...
    if (command[1].match(matchDate)) {
      ...
      await transaction(ScheduleGroup, Schedule, async (ScheduleGroup, Schedule) => {
        ...
        for (let i = 0; i < 7; i++) {
          ...
          const msg = await message.channel.send(`${dateStr} (${weekStr[startDate.getDay()]}) - ${week[startDate.getDay()]}`);
