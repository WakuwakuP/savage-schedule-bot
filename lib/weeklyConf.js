...
module.exports = async function weeklyConf(command, message) {
  ...
  if (command.length === 1) {
    const getWeeklyConf = await WeeklyConf.query().findOne({ serverId: Number(message.guild.id) });
    ...
    if (command[1].match(matchTime) && command[2].match(matchTime)) {
      ...
      try {
        await transaction(WeeklyConf.knex(), async trx => {
          const getWeeklyConf = await WeeklyConf.query(trx).findOne({ serverId: Number(message.guild.id) });
          if (getWeeklyConf == undefined) {
            ...
            console.log(`CREATE: WeeklyConf:${createWeeklyConf.id}`);
          } else {
            ...
            console.log(`UPDATE: WeeklyConf:${getWeeklyConf.id}`);
          }
        });
